-- Optional. Run against the digiproof database, after 02_create_schema.sql.
-- Mirrors api/app/seed.py so you have something to look at in the tables.
--
-- Password hash below is a PLACEHOLDER, not a real bcrypt hash -- fine for
-- browsing data, but if you want to actually log in through the FastAPI
-- app, register these two accounts through the app instead so they get a
-- real bcrypt hash.
